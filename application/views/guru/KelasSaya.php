      ...
      <div class="ibox-content">
        <div class="table-responsive">
          <table id="FDataTable" class="table table-bordered table-hover" style="padding:0px">
            <thead>
              <tr>
                <th style="width: 15%; text-align:center!important">Tahun Ajaran</th>
                <th style="width: 12%; text-align:center!important">Kelas </th>
...
<script>
  $(document).ready(function() {
    ...
    function getAllMapping() {
      return $.ajax({
        url: `<?php echo site_url('ParameterController/getAllV0Mapping/') ?>`,
        'type': 'POST',
    ...
    function renderMapping(data) {
      ...
      Object.values(data).forEach((mapel) => {
        ...
        button = `<a class="detail active btn btn-success btn-sm" href='<?= site_url() ?>AdminController/DetailKelas?id_mapping=${mapel['id_mapping']}'><i class='fa fa-angle-double-right'> Detail </i></a>` 
                
        renderData.push([mapel['nama_jenis_kelas'] + ' ' + mapel['nama_jenis_jurusan'] + ' ' + mapel['sub_kelas'], mapel['deskripsi'] + ' :: Semester ' + mapel['semester'], button]);
      });
